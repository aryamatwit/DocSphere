...
def send_document_to_server(client_socket, text_widget):
    global local_document
    local_document = text_widget.get(1.0, tk.END)  # Get the current content of the text widget
    client_socket.sendall(local_document.encode())  # Send the entire document

# Function to detect any keypress in the text widget and send updates to the server
def on_key_press(event, client_socket, text_widget):
    send_document_to_server(client_socket, text_widget)
...
def start_client():
    try:
        ...
        # Bind key press events to send document updates to the server
        text_widget.bind("<KeyRelease>", lambda event: on_key_press(event, client, text_widget))
